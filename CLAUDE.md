...
- **Microsoft (Azure) Translator** (`api.cognitive.microsofttranslator.com`) — translates single words *and* whole phrases/sentences.
...
Secrets come from `.env` (gitignored) via `python-dotenv`: `BOT_TOKEN`, `AZURE_TRANSLATOR_KEY`,
`AZURE_TRANSLATOR_REGION`, `YANDEX_DICT_API_KEY`. `bot/app.py` exits with code 1 if **any** of these
...
- **`bot/config.py`** — env (`BOT_TOKEN`, Azure Translator + Yandex Dictionary keys), URL constants (`AZURE_TRANSLATE_URL`, `YANDEX_DICT_URL`, `DICT_URL`), `API_TIMEOUT` = 10s, `START_TEXT`, `setup_logging()`.
- **`bot/api.py`** — `fetch_microsoft_translate()` (POST to Azure Translator), `fetch_yandex_dictionary()` + pure `parse_dictionary(allowed_pos)` (translations filtered by POS), `fetch_free_definition()` + pure `pick_definition()` (definition + example).
...
- **word:** `gather(fetch_free_definition → (def, example), fetch_yandex_dictionary(head, allowed_pos) → variants)`; translation = comma-joined variants; if empty, fall back to `fetch_microsoft_translate`.
- **phrase:** `fetch_microsoft_translate` only (no example/definition).
...
- **Azure Translator auth:** headers `Ocp-Apim-Subscription-Key` + `Ocp-Apim-Subscription-Region`, body `[{"Text": ...}]`, response `[{translations:[{text}]}]`. It can echo untranslatable input verbatim, so `fetch_microsoft_translate` treats a result equal to the input (case-insensitive) as `None`.
