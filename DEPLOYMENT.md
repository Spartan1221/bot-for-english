...
2. Отправьте слово `set` — должен прийти ответ из трёх строк (слово с примером / переводы через запятую / английское определение), а под ним три кнопки: «📋 Слово + пример», «📋 Перевод», «📋 Значение». Тап копирует соответствующую часть. Переводы собраны по частям речи (сущ., глаг. и т.д.) одним списком.
3. Отправьте фразу `good morning` — придёт ответ из двух строк (фраза / перевод) и две кнопки; у фразы нет примера и определения.
4. Отправьте `hello world` (фразу) или `123` (цифры) — проверьте реакцию: фраза переводится, цифры/некорректный ввод отвергаются; бессмысленное слово даст «Не удалось перевести …».
5. Логи на сервере: `sudo journalctl -u english-bot -f` (для systemd).
