...
4. Проверьте части речи: `a book`/`an apple`/`the set` → только существительные переводы; `to book`/`to go` → только глагольные; `book` без артикля → все части речи (сущ., глаг., прил.); прилагательное `happy`/`beautiful` без артикля → несколько переводов с разными смыслами.
